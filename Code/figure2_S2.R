...
full.merge.cdmm <- read_tsv(file = "~/Box Sync/Disha/Projects/michelle_raw_data/github_scripts/Data/figure2_S2/full.merge.cdmm.tsv")
...
## controls
controls_cdmm <- read_tsv("./controls_new.txt")
controls_meta_cdmm <- read_tsv("./controls_new_meta.txt")
...
ggplot(final.cdmm, aes(x=time, y=(mean), colour=but)) +
  ...
  facet_wrap(~group_init, scales = "free_y")+
...
ggplot(final_ph, aes(x=time, y=(mean), colour=group)) +
  ...
  #geom_errorbar(aes(ymin=(mean-se), ymax=(mean+se)), width=.01, alpha =0.5) +
  #theme(legend.position = "right") +
  scale_colour_manual(values=c("#801100", "#FC6400","black", "#5D78AB","#87A6BD","#145425", "#80987c"), labels=(final_ph$group)) +
