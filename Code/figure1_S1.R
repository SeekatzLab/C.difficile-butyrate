...
controls <- read_tsv("./controls_new.txt")
controls_meta <- read_tsv("./controls_new_meta.txt")
controls_GC <- subset(controls_meta, controls_meta$medium == "BHI")

controls$time <- time_vec$time
...
ggplot(final_630, aes(x=time, y=(mean), colour=group)) +
  ...
  scale_colour_manual(values=c("#801100", "#FC6400","black", "#5D78AB","#87A6BD","#145425", "#80987c"), labels=(final_630$group)) +
  ylab("OD600") +
  xlab("Time(hrs)") +
  theme_minimal() +
...
#####################
## Fig1b: cd630 colony counts over 6, 12, 18, 24h

### input

all_cfus <- read_excel(path = "/Data/figure1_S1/full_cfu_3_expt.xlsx")
all_cfus_new <- all_cfus %>% 
  filter(group %in% c("Ace25", "Ace5",  "BHI", "But25", "But5",  "Pro25", "Pro5")) %>%
  droplevels()

all_cfus_630 <- all_cfus %>% 
  #filter(expID %in% c("SCFACU21", "SCFACU22", "SCFACU29")) %>%
  filter(group %in% c("Ace25", "Ace5",  "BHI", "But25", "But5",  "Pro25", "Pro5")) %>%
  droplevels() %>%
  group_by(time, group) %>%
...
ggplot(all_cfus_new, aes(x=as.factor(time), y=(CFU), color=group)) + geom_point(position = position_dodge(width = 0.75, preserve = "total"), alpha=0.3) +
  #geom_line() + 
  #facet_grid(~time) +
...
summary(aov_cfu_12)
TukeyHSD(aov_cfu_12)
DunnettTest(x=all_cfus_12$CFU, g=all_cfus_12$group, "BHI")
...
#               Df Sum Sq Mean Sq F value Pr(>F)  
#  group        6  7.052  1.1753   2.325 0.0478 *
#  Residuals   47 23.762  0.5056                 
#---
#  Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
#Fit: aov(formula = CFU ~ group, data = all_cfus_18)

#$group
#diff        lwr        upr     p adj
#Ace5-BHI    -0.13804427 -1.2328570  0.9567685 0.9997031
#Ace25-BHI    0.60488635 -0.4899264  1.6996991 0.6184978
#Pro5-BHI    -0.33414084 -1.4289536  0.7606719 0.9639020
#Pro25-BHI   -0.38016192 -1.4749746  0.7146508 0.9339253
#But5-BHI    -0.64943092 -1.6149650  0.3161031 0.3859673
#But25-BHI   -0.44644476 -1.4119788  0.5190893 0.7862527
#Ace25-Ace5   0.74293062 -0.5212502  2.0071115 0.5483702
#Pro5-Ace5   -0.19609657 -1.4602774  1.0680843 0.9990260
#Pro25-Ace5  -0.24211766 -1.5062985  1.0220632 0.9968280
#But5-Ace5   -0.51138665 -1.6654206  0.6426473 0.8173178
#But25-Ace5  -0.30840049 -1.4624344  0.8456334 0.9813306
#Pro5-Ace25  -0.93902719 -2.2032080  0.3251536 0.2715215
#Pro25-Ace25 -0.98504828 -2.2492291  0.2791326 0.2216588
#But5-Ace25  -1.25431727 -2.4083512 -0.1002833 0.0251290
#But25-Ace25 -1.05133112 -2.2053651  0.1027028 0.0957018
#Pro25-Pro5  -0.04602108 -1.3102019  1.2181598 0.9999998
#But5-Pro5   -0.31529007 -1.4693240  0.8387439 0.9791194
#But25-Pro5  -0.11230392 -1.2663379  1.0417300 0.9999345
#But5-Pro25  -0.26926899 -1.4233029  0.8847649 0.9907708
#But25-Pro25 -0.06628284 -1.2203168  1.0877511 0.9999971
#But25-But5   0.20298615 -0.8292132  1.2351855 0.9963312

DunnettTest(x=all_cfus_18$CFU, g=all_cfus_18$group, "BHI")
...
summary(aov_cfu_24)
#               Df Sum Sq Mean Sq F value   Pr(>F)    
#  group        6  12.17  2.0285   5.266 0.000211 ***
#  Residuals   59  22.73  0.3852                     
#---
#  Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
#Fit: aov(formula = CFU ~ group, data = all_cfus_24)

#$group
#                 diff        lwr          upr     p adj
#Ace5-BHI    -0.16642309 -1.0017949  0.668948683 0.9963174
#Ace25-BHI   -0.05471807 -0.8900898  0.780653697 0.9999942
#Pro5-BHI    -0.38948208 -1.2248539  0.445889689 0.7872201
#Pro25-BHI   -0.80215524 -1.6375270  0.033216533 0.0677254
#But5-BHI    -0.83196955 -1.6673413  0.003402223 0.0516082
#But25-BHI   -1.20849878 -2.0438705 -0.373127008 0.0008222
#Ace25-Ace5   0.11170501 -0.7813450  1.004755001 0.9997367
#Pro5-Ace5   -0.22305899 -1.1161090  0.669990992 0.9876029
#Pro25-Ace5  -0.63573215 -1.5287821  0.257317836 0.3256906
#But5-Ace5   -0.66554646 -1.5585964  0.227503526 0.2734606
#But25-Ace5  -1.04207569 -1.9351257 -0.149025704 0.0123321
#Pro5-Ace25  -0.33476401 -1.2278140  0.558285978 0.9114978
#Pro25-Ace25 -0.74743716 -1.6404872  0.145612822 0.1591985
#But5-Ace25  -0.77725147 -1.6703015  0.115798512 0.1280419
#But25-Ace25 -1.15378071 -2.0468307 -0.260730718 0.0038548
#Pro25-Pro5  -0.41267316 -1.3057231  0.480376830 0.7940561
#But5-Pro5   -0.44248747 -1.3355375  0.450562521 0.7363786
#But25-Pro5  -0.81901670 -1.7120667  0.074033290 0.0927486
#But5-Pro25  -0.02981431 -0.9228643  0.863235677 0.9999999
#But25-Pro25 -0.40634354 -1.2993935  0.486706446 0.8055142
#But25-But5  -0.37652923 -1.2695792  0.516520756 0.8551742
...
#$BHI
#                diff     lwr.ci      upr.ci    pval    
#Ace5-BHI  -0.16642309 -0.8958442  0.56299803 0.98154    
#Ace25-BHI -0.05471807 -0.7841392  0.67470304 0.99996    
#Pro5-BHI  -0.38948208 -1.1189032  0.33993903 0.55381    
#Pro25-BHI -0.80215524 -1.5315764 -0.07273412 0.02516 *  
#But5-BHI  -0.83196955 -1.5613907 -0.10254843 0.01865 *  
#But25-BHI -1.20849878 -1.9379199 -0.47907766 0.00025 ***
...
## Fig1c: growth curves for butyrate concentration gradient
...
# edits were made to put triplicates together in Excel
combo4_edit_x <- read_tsv(file= "./combo3_gcvr_dr_edit.txt")
...
all_cfus <- read_excel(path = "./full_cfu_3_expt.xlsx")
all_cfus <- all_cfus[,-5]
added_data <- read_xlsx("./full_fig2_cfus.xlsx")
...
# fig 1d: cfus for dose response

cfus_dr <- read_xlsx(path = "./dose_but_3_expt.xlsx")
cfus_dr$group <- factor(cfus_dr$group, levels = c("BHI","But5", "But10", "But25", "But50"))
ggplot(cfus_dr, aes(x=group, y=(CFU), color=type)) + geom_point(position = position_dodge(width = 0.75, preserve = "total"), alpha=0.3) +
  #geom_line() + 
  #facet_grid(~time) +
  geom_boxplot(outlier.shape = NA) +
  theme_bw() + 
...
DunnettTest(x=cfus_dr_heat$CFU, g=cfus_dr_heat$group, "BHI")
...
#$BHI
#               diff     lwr.ci   upr.ci    pval    
#But10-BHI 0.1478881 -1.5769962 1.872772 0.99837    
#But25-BHI 2.4898759  0.7649915 4.214760 0.00261 ** 
#But5-BHI  0.4889350 -1.2359494 2.213819 0.88146    
#But50-BHI 2.9407486  1.2158642 4.665633 0.00043 ***

#---
#Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

cfus_dr_unheat <- subset(cfus_dr, !(cfus_dr$type == "heat"))
DunnettTest(x=cfus_dr_unheat$CFU, g=cfus_dr_unheat$group, "BHI")
## no significance
...
ggplot(final_VPI, aes(x=time, y=(mean), colour=group)) +
  geom_smooth(size = 0.5, se = FALSE) + 
  #geom_point(size=.1, alpha = 0.5) +
  #geom_errorbar(aes(ymin=(mean-se), ymax=(mean+se)), width=.01, alpha =0.5) +
  theme(legend.position = "right") +
  scale_colour_manual(values=c("#801100", "#FC6400","black","#5D78AB","#87A6BD","#145425", "#80987c"), labels=(final_VPI$group)) +
  ylab("OD600") +
  xlab("Time(hrs)") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(0, 24, by = 4)) +
...
combo4_VPI <- read_tsv(file= "./combo3_VPI10463_gcvr_edit.txt")
...
ggplot(final_R2, aes(x=time, y=(mean), colour=group)) +
  geom_smooth(size = 0.5, se = FALSE) + 
  #geom_point(size=.1, alpha = 0.5) +
  #geom_errorbar(aes(ymin=(mean-se), ymax=(mean+se)), width=.01, alpha =0.5) +
  #theme(legend.position = "right") +
  scale_colour_manual(values=c("#801100", "#FC6400","black", "#5D78AB","#87A6BD","#145425", "#80987c"), labels=(final_R2$group)) +
  ylab("OD600") +
  xlab("Time(hrs)") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(0, 24, by = 4)) +
...
##Changes made in excel: put time in front, removed the V1, V2 (row 1)
combo4_R2 <- read_tsv(file= "./combo3_R20291_gcvr_edit.txt")
