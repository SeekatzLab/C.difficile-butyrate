...
ggplot(scfa_24, aes(x=group, y=CFU, color=group)) + geom_point(size =0.5, position = position_jitter(width = 0.4)) +
  #geom_point(size =0.5) + 
  #facet_grid(~time) +
...
ggplot(cfus_630_heat, aes(x=as.factor(time), y=CFU, color=group)) + geom_point(size =0.5, position = position_jitterdodge(jitter.width = 0.5)) +
  #geom_point(size =0.5) + 
  #facet_grid(~time) +
  geom_boxplot(alpha =0.4, size = 0.3, aes(fill = group), outlier.shape = NA) +
  theme_bw() +
...
ggplot(spore_eff_4, aes(x=media, y=mean, fill=media)) + geom_col() +
  theme_bw() +
